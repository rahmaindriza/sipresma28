...
<div class="space-y-6">
    <div class="flex items-center space-x-3">
        <a href="{{ route('guru.index') }}" class="p-2 bg-slate-900 border border-slate-800 text-slate-400 hover:text-white rounded-xl transition">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
        </a>
        <div>
            <h3 class="text-xl font-bold text-white">Input Komponen Nilai</h3>
            <p class="text-xs text-slate-400 mt-1">Kelas: <span class="text-white font-semibold">{{ $kelas->nama_kelas }}</span> | Mapel: <span class="text-white font-semibold">{{ $mapel->nama_mapel }}</span></p>
        </div>
    </div>

    <!-- Info card -->
    <div class="p-4 rounded-2xl bg-blue-950/40 border border-blue-900/40 flex items-start space-x-3">
        <div class="p-2 bg-blue-600/20 text-blue-400 rounded-lg shrink-0">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        </div>
        <div class="text-xs text-slate-300 leading-relaxed">
            <span class="font-bold text-blue-400">Aturan Bobot Penilaian:</span> Tugas: 20%, Ulangan Harian (UH): 30%, UTS: 20%, UAS: 30%.
            KKM standar kelulusan adalah <span class="font-bold text-white">{{ $mapel->kkm }}</span>. Nilai akhir dihitung otomatis di backend saat disimpan, namun kolom estimasi di bawah akan menampilkan perhitungan secara instan untuk membantu Anda.
        </div>
    </div>
    ...
    <form action="{{ route('guru.grades.store', $assignment->id) }}" method="POST" class="space-y-6">
        ...
        <div class="flex justify-end space-x-3">
            <a href="{{ route('guru.index') }}" class="px-5 py-3 bg-slate-900 border border-slate-800 hover:bg-slate-850 text-slate-300 font-semibold rounded-xl text-xs transition">Batal</a>
