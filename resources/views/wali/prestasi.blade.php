...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-[var(--border-light)] pb-4">
        ...
        <a href="{{ route('wali.prestasi.create') }}" class="mt-4 sm:mt-0 px-4 py-2 text-white font-semibold rounded-xl text-xs transition flex items-center shadow-sm" style="background-color: var(--primary-burgundy) !important; border: none; box-shadow: 0 4px 10px rgba(61, 90, 128, 0.25);">
                ...
                <tbody class="divide-y divide-[var(--border-light)]">
                    ...
                    <tr class="hover:bg-[var(--accent-table-hover)] transition duration-150">
                        ...
                        <td class="py-3.5 px-4 text-right space-x-1 whitespace-nowrap">
                            ...
                            <form action="{{ route('wali.prestasi.destroy', $p->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
...
<script>
    ...
    function editPrestasi(prestasi) {
        const actionUrl = `/wali/prestasi/${prestasi.id}`;
