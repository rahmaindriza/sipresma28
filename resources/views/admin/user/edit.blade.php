...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-xl font-bold text-white">Edit Data Pengguna</h3>
            <p class="text-xs text-slate-400 mt-1">Perbarui data login atau hak akses pengguna.</p>
        </div>
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-5">
            ...
            <div class="row g-4">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" required class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-blue-500 transition text-sm">
                </div>
                <div class="col-md-6">
                    <label class="block text-xs font-semibold text-slate-350 uppercase tracking-wider mb-2">Email</label>
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" required class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-blue-500 transition text-sm">
                </div>
            </div>
            ...
            <div class="row g-4">
                <div class="col-md-6">
                    ...
                    <input type="text" name="username" value="{{ old('username', $user->username) }}" required class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-blue-500 transition text-sm">
                </div>
                <div class="col-md-6">
                    <label class="block text-xs font-semibold text-slate-350 uppercase tracking-wider mb-2">Password (Kosongkan jika tidak ingin diubah)</label>
                    <input type="password" name="password" autocomplete="new-password" class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-blue-500 transition text-sm">
                </div>
            </div>
                ...
                <div class="col-md-6">
                    <label class="block text-xs font-semibold text-slate-350 uppercase tracking-wider mb-2">Role Akses</label>
                     <select name="role" id="edit-role" required class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-blue-500 transition text-sm">
                        ...
                        <option value="guru_mapel" {{ old('role', $user->role) == 'guru_mapel' ? 'selected' : '' }}>Guru Mata Pelajaran</option>
            ...
            <div class="row g-4">
                <div class="col-md-6" id="edit-kelas-select-container" style="display: {{ old('role', $user->role) == 'wali_kelas' ? 'block' : 'none' }};">
                    <label class="block text-xs font-semibold text-slate-350 uppercase tracking-wider mb-2">Kelas Diampu</label>
                    <select name="kelas_id" id="edit-kelas-select" class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-blue-500 transition text-sm" {{ old('role', $user->role) == 'wali_kelas' ? 'required' : '' }}>
            ...
            <div class="flex justify-end space-x-3 pt-4 border-t border-slate-800/60 mt-6">
                ...
                <button type="submit" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl text-xs transition shadow-sm">Perbarui Data</button>
            </div>
        </form>
    </div>
</div>
